...
export async function apiCall(url) {
    try {
        ...
        // if failed
        if (!response.ok) {
            throw Error(`Request Failed, Status Code: ${response.status}, Message: ${response.statusText}`);
        }
        let usersData = await response.json();
        //test
        // console.log(jsResponse);
        console.log(`Request Successful, Status Code: ${response.status}, Message: ${response.statusText}`);
...
export async function handleData(url, datafetcher) {
    try {
        console.log(`\nURL: ${url}\n`);
        // call
        //! now how works
        ...
        /*
             ...
             *   It slices the box open and takes the plain data out of the box.
                ...
                */
        let data = await datafetcher(url);
        return data;
    }
    catch (error) {
        if (error instanceof Error) {
...
//* TEST
// instance for users
handleData("https://jsonplaceholder.typicode.com/users", apiCall);
// instance for posts
handleData("https://jsonplaceholder.typicode.com/posts", apiCall);
//# sourceMappingURL=typed-api-warpper.js.map